...
% Last Modified by GUIDE v2.5 25-Nov-2020 16:22:42
...
if (comprobp == 1)
    ...
    if (comprobp == 1 && compeade == 0 && compeatr == 0 && compeadeatr == 0)
    der = 0;
    int = 0;
    cont = tf(str2num(Kp),1);
    compensad=1;
...
if (comprobi == 1)
    ...
    if (comprobi == 1 && compeade == 0 && compeatr == 0 && compeadeatr == 0)
    der = 0;
    int = str2num(Kp)/(str2num(T));
    cont = tf([str2num(Kp)*str2num(T) str2num(Kp)],[str2num(T) 0]);
    compensad=1;
...
if (comprobd == 1)
    ...
    if (comprobd == 1 && compeade == 0 && compeatr == 0 && compeadeatr == 0)
    int = 0;
    der = str2num(Kp)*str2num(T);
    cont = tf ([str2num(Kp)*str2num(T) 1],1);
    compensad=1;
...
if (bool == 1)
    ...
    cla reset;
    for i=1:n
        nuevproc = (((Act1*pla1*ks(i)*compensad))/(1+Act1*pla1*sens1*ks(i)*compensad));
        step(nuevproc);
    ...
    nuevK = i; 
    contad = contad+1;
    nuevproc = (((Act1*pla1*cont*nuevK*compensad))/(1+Act1*pla1*sens1*cont*nuevK*compensad));
    paramN = stepinfo(nuevproc);
...
% Si presionamos controlador proporcional, habilitamos la visibilidad del
% slider K y apagamos el de T
set(handles.Ktext,'visible','on');
set(handles.Kslider,'visible','on');
...
% handles    structure with handles and user data (see GUIDATA)
global Kslider;
Kslider = get(handles.Kslider,'Value');
set(handles.Kcontr,'string',Kslider);
...
% handles    structure with handles and user data (see GUIDATA)
global Tcon;
Tcon = get(handles.Tslider,'Value');
set(handles.Tcontrolador,'string',Tcon);
