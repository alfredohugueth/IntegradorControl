function [salida,pantalla,rang]=RouthSolver(entrada)
...
for i = 1:1:length(coef)
    ...
    condi3 = has(tems,td);
    if (condi)
        mostrar = ('la expresion contiene k');
        res = solve(tems>0,k,'Returnconditions',true);
        tem = length(res.conditions);
        if(tem == 0 )
            mostrar2=('El sistema no tiene solución');
            rango(j) = x>0;
            j = j+1;
        else
            rango(j) = res.conditions;
            j = j+1;
        end
        
    end
    if(condi2)
        res = solve(tems>0,ti,'Returnconditions',true)
        tem = length(res.conditions);
        if(tem == 0)
            rango(j) = "x>0";
            j = j+1;
      ...
      if(condi3)
         res = solve(tems>0,td,'Returnconditions',true);
         tem = length(res.conditions);
        if(tem == 0)
            rango(j) = "x>0";
            j = j+1;
...
rang = rango;
%%rango=solve(princip,k,'Returnconditions',true)
%%rang = rango.parameters;